...
| **微信连接**      | 扫码登录微信机器人，在微信中与 AI 实时对话，支持主动消息推送  |
| **图片识别**      | 支持多模态视觉模型识别图片内容，非视觉模型优雅降级        |
| **URL 内容理解**  | 自动抓取对话中分享的链接内容，AI 能理解网页信息         |
| **情感表情动效**    | 聊天时伴随 GIF 动效表达情绪（开心/生气/害羞/撒娇……）    |
| **纪念日管理**     | 自动记录重要日子，AI 会主动送上祝福                |
| **日常生活模拟**    | 每个 AI 拥有独立日程表，回复自然融入"现在在做什么"       |
| **隐私优先**      | 所有数据本地存储，无需注册第三方账号                 |
| **模型自由切换**    | 兼容所有 OpenAI 协议模型，UI 界面运行时热切换，支持视觉模型 |
...
（初次登入管理员账号，需要进行注册，在登入界面点击注册，账号为admin888，密码你可以任意输入）
...
- 忙闲状态管理
- 信念系统 + 分手管理
- 人设 Lorebook 引擎（关键词触发 + 条目管理）
- 模型运行时热切换
- 管理后台增强（人设模板编辑、头像上传、Lorebook 管理、视觉模型配置）
